...
public class DeleteProductHandler : IRequestHandler<DeleteProductCommand, Result<Guid>>
{
    ...
    public async Task<Result<Guid>> Handle(DeleteProductCommand request, CancellationToken cancellationToken)
    {
        ...
        if (product == null)
        {
            var result = Result.Failure<Guid>(new Error("DeleteProduct.NotFound", "Product not found."));
        ...
        _logger.LogInformation("{Class}: {Method} - Success, deleted {orderId}.", nameof(GetOrdersHandler), nameof(Handle), product.ProductId);
